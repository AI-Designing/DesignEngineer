...
def execute_freecad_component(code_file: Path):
    ...
    logger.info(f"🔧 Executing FreeCAD component: {code_file.name}")
...
def main():
    ...
    logger.info("🎯 Direct Component Generation with DeepSeek R1")
    logger.info("=" * 60)
    ...
    for i, component in enumerate(components, 1):
        logger.info(
            f"\n🔨 Generating Component {i}/{len(components)}: {component['name']}"
        )
        logger.info("-" * 40)
        ...
        try:
            ...
            if output_dir:
                ...
                code_file = output_dir / "generated_component.py"
                if code_file.exists():
                    logger.info("🚀 Executing generated component...")
                    exec_success = execute_freecad_component(code_file)
                    if exec_success:
                        logger.info("🎨 FreeCAD execution completed successfully")
                    else:
            ...
            logger.error(f"❌ Error generating {component['name']}: {e}")
            failed += 1

        # Brief pause between generations
        if i < len(components):
            logger.info("⏳ Brief pause before next generation...")
            time.sleep(5)
    ...
    logger.info(f"📈 Success Rate: {(successful/len(components)*100):.1f}%")
