    ...
    try:
        ...
        class MockWorkflowOrchestrator:
            ...
            def plan_execution_sequence(self, workflow_steps):
                ...
                print(
                    f"📋 Mock: Planning execution sequence for {len(workflow_steps)} steps"
                )
...
def test_complex_workflow_command():
    ...
    print(
        "🎯 Testing Complex Workflow: 'Create a bracket with 4 mounting holes and fillets'"
    )
    print("=" * 80)
    ...
    try:
        ...
        if step_results:
            ...
            for step in step_results:
                ...
                print(
                    f"  {status_icon} Step {step.get('step_number', '?')}: {step.get('step_name', 'Unknown')}"
                )
                print(
                    f"     Time: {step.get('execution_time', 0):.2f}s | Result: {step.get('result', 'No result')}"
                )
        ...
        if success:
            ...
            print(
                f"✅ All {result.get('completed_steps', 0)} steps completed successfully"
            )
        else:
