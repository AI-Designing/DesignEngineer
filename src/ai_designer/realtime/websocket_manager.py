...
class WebSocketManager:
    ...
    def send_state_change(self, state_data: Dict[str, Any], session_id: str = None):
        ...
        message = WebSocketMessage(
            ...
            data={"state": state_data, "timestamp": datetime.now().isoformat()},
            timestamp=datetime.now(),
            session_id=session_id,
        )
    ...
    def send_user_notification(
        self, notification: str, notification_type: str = "info", session_id: str = None
    ):
    ...
    async def _send_queued_message(
        self, send_type: str, target: str, message: WebSocketMessage
    ):
...
class ProgressTracker:
    ...
    def update_progress(self, command_id: str, step: int = None, message: str = None):
        ...
        self.ws_manager.send_progress_update(
            ...
            status="running",
            message=message
            or f"Step {tracker['current_step']}/{tracker['total_steps']}",
            session_id=tracker["session_id"],
            estimated_remaining=int(remaining) if remaining else None,
        )

    def complete_tracking(
        self, command_id: str, success: bool = True, message: str = None
    ):
        ...
        self.ws_manager.send_progress_update(
            ...
            message=message
            or ("Command completed successfully" if success else "Command failed"),
            session_id=tracker["session_id"],
        )
    ...
    def fail_tracking(self, command_id: str, error_message: str):
        ...
        self.complete_tracking(
            command_id, success=False, message=f"Error: {error_message}"
        )
