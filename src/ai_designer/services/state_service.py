...
class FreeCADStateService:
    ...
    def get_current_analysis(
        ...
        latest_key = self.state_cache.get_latest_analysis(
            document_name, "full_analysis"
        )
        if latest_key:
    ...
    def get_state_history(
        self, document_name: str = None, session_id: str = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    def compare_states(self, key1: str, key2: str) -> Dict[str, Any]:
        ...
        for key in all_keys:
            ...
            if val1 != val2:
                comparison["differences"][key] = {"state1": val1, "state2": val2}

        return comparison

    def monitor_state_changes(
        self, document_name: str, target_conditions: Dict[str, bool]
    ) -> Dict[str, Any]:
        """
    ...
    def export_data(
        self, document_name: str = None, format: str = "json"
        ...
        for key in state_keys:
            state_data = self.state_cache.retrieve_with_metadata(key)
            if state_data:
                export_data["states"].append({"key": key, "data": state_data})
        ...
        for key in analysis_keys:
            ...
            if analysis_data:
                export_data["analyses"].append({"key": key, "data": analysis_data})
