            ...
            try:
                ...
                if pid and self._is_process_running(pid):
                    ...
                    if self._test_connection(port):
                        print(f"🔗 Found existing persistent GUI (PID: {pid}, Port: {port})")
                        ...
                        print(f"⚠️  Existing GUI process found but not responding, will restart")
        ...
        try:
            ...
            if self.websocket_manager:
                self.websocket_manager.send_error(
                    f"GUI communication error: {str(e)}",
                    session_id=self.session_id
                )
            return False
    ...
    def stop_gui(self):
        ...
        try:
            if self.gui_process and self.is_gui_running():
                ...
                if self.websocket_manager:
                    self.websocket_manager.send_user_notification(
                        "Persistent FreeCAD GUI stopped",
                        "info",
                        self.session_id
                    )
