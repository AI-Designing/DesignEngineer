...
class DeepSeekR1Client:
    ...
    def _verify_connection(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️ DeepSeek R1 health check failed: {response.status_code}")
                return False
    ...
    def generate_complex_part(self, 
                            requirements: str,
                            mode: DeepSeekMode = DeepSeekMode.REASONING,
                            context: Optional[Dict[str, Any]] = None,
                            constraints: Optional[Dict[str, Any]] = None) -> DeepSeekResponse:
        """
        ...
        try:
            ...
            prompt = self._build_complex_part_prompt(requirements, mode, context, constraints)
            ...
            validation_result = self._validate_generated_code(parsed_response.generated_code)
            ...
            # Store in history
            self._store_generation_history(requirements, parsed_response, execution_time)
    ...
    def _build_complex_part_prompt(self, 
                                 requirements: str,
                                 mode: DeepSeekMode,
...
Please generate production-ready code that creates the requested complex part."""
    ...
    def _make_deepseek_request(self, prompt: str, mode: DeepSeekMode) -> Dict[str, Any]:
        ...
        if response.status_code != 200:
            raise Exception(f"DeepSeek R1 request failed: {response.status_code} - {response.text}")
    ...
    def _parse_deepseek_response(self, response: Dict[str, Any]) -> DeepSeekResponse:
        ...
        try:
            ...
            confidence_score = self._calculate_confidence_from_content(content, reasoning_chain)
    ...
    def _extract_code_from_content(self, content: str) -> str:
        ...
        if "```" in content:
            ...
            if end != -1:
                ...
                if any(keyword in code for keyword in ["import", "def", "FreeCAD", "Part"]):
                    return code
        ...
        for line in lines:
            line = line.strip()
            if any(keyword in line for keyword in ["import FreeCAD", "import Part", "def ", "doc = "]):
            ...
            if in_code_section:
                ...
                if line and not line.startswith('#') and not any(
                    char in line for char in ['=', '(', ')', '.', 'import', 'def', 'if', 'for', 'try']
                ):
                    if not any(keyword in line for keyword in ["FreeCAD", "Part", "doc", "obj"]):
                        break
        
        return '\n'.join(code_lines) if code_lines else content
    
    def _calculate_confidence_from_content(self, content: str, reasoning_chain: List[ReasoningStep]) -> float:
    ...
    def _calculate_confidence(self, choice: Dict, reasoning_chain: List[ReasoningStep]) -> float:
    ...
    def _analyze_code_complexity(self, code: str) -> Dict[str, Any]:
        ...
        boolean_ops = len([line for line in lines if any(op in line for op in ['fuse', 'cut', 'common'])])
        advanced_features = len([line for line in lines if any(
            feature in line for feature in ['Sketch', 'Pad', 'Pocket', 'Fillet', 'Chamfer']
        )])
    ...
    def _generate_optimization_suggestions(self, code: str, complexity: Dict[str, Any]) -> List[str]:
        """Generate optimization suggestions for the code"""
        suggestions = []
        
        # Check for common optimization opportunities
        if complexity['object_count'] > 10:
            suggestions.append("Consider grouping related objects for better organization")
        ...
        if not any('def ' in line for line in lines):
            suggestions.append("Consider organizing code into functions for reusability")
    ...
    def _validate_generated_code(self, code: str) -> Dict[str, Any]:
        ...
        validation_result['executable'] = (
            ...
            validation_result['imports_valid'] and 
            validation_result['freecad_valid']
        )
    ...
    def _update_metrics(self, response: DeepSeekResponse, execution_time: float):
        ...
        self.performance_metrics['average_response_time'] = (
            (self.performance_metrics['average_response_time'] * (total - 1) + execution_time) / total
        )
        
        self.performance_metrics['average_confidence'] = (
            (self.performance_metrics['average_confidence'] * (total - 1) + response.confidence_score) / total
        )
    
    def _store_generation_history(self, requirements: str, response: DeepSeekResponse, execution_time: float):
    ...
    def get_reasoning_insights(self) -> Dict[str, Any]:
        ...
        avg_reasoning_steps = sum(entry['reasoning_steps'] for entry in recent_entries) / len(recent_entries)
        avg_complexity = sum(entry['complexity_score'] for entry in recent_entries) / len(recent_entries)
        ...
        high_confidence_count = len([e for e in recent_entries if e['confidence'] > 0.8])
...
class DeepSeekIntegrationManager:
    ...
    def _select_optimal_mode(self, nl_command: str, state) -> str:
        ...
        complexity_indicators = [
            'complex', 'advanced', 'intricate', 'assembly', 'gear', 'mechanism',
            'optimization', 'parametric', 'pattern', 'array', 'sweep', 'loft'
        ]
        
        command_lower = nl_command.lower()
        complexity_score = sum(1 for indicator in complexity_indicators if indicator in command_lower)
    ...
    def _generate_with_deepseek(self, nl_command: str, state) -> str:
        ...
        if any(word in nl_command.lower() for word in ['creative', 'innovative', 'artistic']):
            mode = DeepSeekMode.CREATIVE
        elif any(word in nl_command.lower() for word in ['precise', 'accurate', 'technical']):
...
"""
    ...
    def _merge_solutions(self, deepseek_code: str, fallback_code: str, nl_command: str) -> str:
        ...
        if any(word in nl_command.lower() for word in ['complex', 'advanced', 'assembly']):
            return deepseek_code
        else:
    ...
    def get_integration_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.integration_metrics,
            ...
            'deepseek_usage_rate': self.integration_metrics['deepseek_requests'] / max(1, total_requests),
            'fallback_usage_rate': self.integration_metrics['fallback_requests'] / max(1, total_requests),
            'hybrid_usage_rate': self.integration_metrics['hybrid_requests'] / max(1, total_requests),
