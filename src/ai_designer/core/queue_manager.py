...
class CommandQueue:
    ...
    def add_command(self, command: QueuedCommand) -> str:
        ...
        with self._lock:
            ...
            print(f"🔄 Command queued: {command.id[:8]} (Priority: {command.priority.name})")
...
class QueueManager:
    ...
    def submit_command(self, user_input: str, command: str, session_id: str = None,
                      priority: CommandPriority = CommandPriority.NORMAL,
        ...
        try:
            ...
            def timeout_handler():
                ...
                if command.status == CommandStatus.RUNNING:
                    ...
                    command.error = f"Command timed out after {command.timeout_seconds} seconds"
                    command.completed_at = datetime.now()
            ...
            if command.retry_count < command.max_retries:
                ...
                print(f"🔄 Retrying command {command.id[:8]} (attempt {command.retry_count + 1}/{command.max_retries + 1})")
                
                # Re-queue for retry
                self.command_queue.add_command(command)
                return
    
    def get_command_result(self, command_id: str, timeout: int = 30) -> Optional[QueuedCommand]:
