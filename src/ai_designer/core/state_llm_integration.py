...
try:
    ...
    from .complex_shape_generator import ComplexityLevel, ComplexShapeGenerator
...
class StateLLMIntegration:
        ...
        try:
            ...
            if not validation_result["valid"]:
                ...
                return self._handle_validation_failure(
                    validation_result, decision_context
                )
    ...
    def _prepare_decision_context(
        self, user_input: str, session_id: str, intent_result: Dict[str, Any]
    ) -> DecisionContext:
        ...
        if self.state_service:
            try:
                ...
                if not current_state:
                    ...
                    current_state = (
                        self.state_service.analyze_and_cache(session_id) or {}
                    )
            except Exception as e:
...
"""
    ...
    def _validate_decision(
        self, decision: LLMDecision, context: DecisionContext
    ) -> Dict[str, Any]:
    ...
    def _execute_decision(
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "command": decision.command}

    def _update_state_after_execution(
        self, execution_result: Dict[str, Any], session_id: str
    ):
    ...
    def _handle_validation_failure(
        self, validation_result: Dict[str, Any], context: DecisionContext
    ) -> Dict[str, Any]:
...
class EnhancedStateLLMIntegration(StateLLMIntegration):
    ...
    def __init__(
        ...
        super().__init__(
            state_service, llm_client, command_executor, queue_manager, intent_processor
        )
    ...
    def process_complex_shape_request(
        ...
        try:
            ...
            generation_strategy = self._determine_generation_strategy(
                complexity_analysis, user_input
            )

            # Step 3: Enhanced state analysis
            enhanced_context = self._create_enhanced_context(
                user_input, session_id, complexity_analysis
            )

            if generation_strategy == GenerationStrategy.DECOMPOSED:
                # Use complex shape generator
                result = self._process_with_complex_generator(
                    user_input, session_id, enhanced_context
                )
            else:
                # Use enhanced iterative processing
                result = self._process_with_iterative_enhancement(
                    user_input, session_id, enhanced_context
                )
    ...
    def _analyze_complexity_requirements(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            if self.llm_client:
                ...
                complexity_data["keywords"] = self._extract_complexity_keywords(
                    user_input
                )
    ...
    def _determine_generation_strategy(
        self, complexity_analysis: Dict[str, Any], user_input: str
    ) -> GenerationStrategy:
        """
        ...
        decomposition_recommended = complexity_analysis.get(
    ...
    def _create_enhanced_context(
        self, user_input: str, session_id: str, complexity_analysis: Dict[str, Any]
    ) -> LLMDecisionContext:
        """
        ...
        constraint_analysis = self._analyze_constraints(
            current_state, complexity_analysis
        )
        ...
        generation_goals = self._extract_generation_goals(
            user_input, complexity_analysis
        )
    ...
    def _process_with_complex_generator(
        self, user_input: str, session_id: str, context: LLMDecisionContext
    ) -> Dict[str, Any]:
        """
        ...
        complexity_level = context.complexity_analysis.get(
    ...
    def _process_with_iterative_enhancement(
        self, user_input: str, session_id: str, context: LLMDecisionContext
    ) -> Dict[str, Any]:
        """
        ...
        while current_iteration < max_iterations:
            ...
            execution_result = self._execute_with_state_monitoring(
                enhanced_decision, session_id
            )
            ...
            iteration_result = {
                ...
                "decision": (
                    enhanced_decision.__dict__
                    if hasattr(enhanced_decision, "__dict__")
                    else enhanced_decision
                ),
                "execution": execution_result,
    ...
    def _get_enhanced_llm_decision(
        self, context: LLMDecisionContext
    ...
    def _execute_with_state_monitoring(
        self, decision: EnhancedDecisionResult, session_id: str
    ) -> Dict[str, Any]:
        """
        ...
        for step in decision.action_plan:
            print(
                f"⚡ Executing step {step.get('step', 'unknown')}: {step.get('description', '')}"
            )
            ...
            # Validate step
            validation_result = self._validate_step_execution(
                step, step_results, pre_state, post_state
            )
            ...
            if not validation_result.get("success", False) and validation_result.get(
                "critical", False
            ):
                break
        ...
        return {
            ...
            "overall_success": all(
                step["validation"].get("success", False) for step in execution_results
            ),
        }

    def _get_progress_feedback(
        self,
        context: LLMDecisionContext,
        decision: EnhancedDecisionResult,
    ...
    def _add_continuous_monitoring(
        self, generation_result: Dict[str, Any], session_id: str
    ) -> Dict[str, Any]:
    ...
    def _check_completion_criteria(
        ...
        return progress >= 90.0 or feedback.get("approach_recommendation", {}).get(
    ...
    def _validate_step_execution(
        ...
        objects_created = post_state.get("object_count", 0) - pre_state.get(
            "object_count", 0
        )
    ...
    def _create_fallback_enhanced_decision(
        self, context: Optional[LLMDecisionContext]
    ) -> EnhancedDecisionResult:
