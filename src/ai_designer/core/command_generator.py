...
class StateAwareCommandGenerator:
    ...
    def generate_command_with_state(self, user_input: str, intent_data: Dict[str, Any], 
    ...
    def _get_comprehensive_state(self, session_id: str = None) -> Dict[str, Any]:
        ...
        state_context['capabilities'] = self._get_current_capabilities(state_context['current_state'])
        state_context['constraints'] = self._get_current_constraints(state_context['current_state'])
        
        return state_context
    
    def _generate_with_llm(self, user_input: str, intent_data: Dict[str, Any], 
        ...
        try:
            ...
            intent_type = intent_data.get('intent_type', 'general_command')
            prompt_template = self.prompt_templates.get(intent_type, self.prompt_templates['analyze_and_decide'])
            ...
            prompt = prompt_template.format(
                ...
                session_context=json.dumps(state_context.get('session_context', {}), indent=2)
            )
    ...
    def _generate_with_rules(self, user_input: str, intent_data: Dict[str, Any], 
                            state_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_create_object_command(self, user_input: str, current_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_analyze_command(self, current_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_save_command(self, user_input: str, current_state: Dict[str, Any]) -> Dict[str, Any]:
...
'''
    ...
    def _format_state_for_llm(self, state_context: Dict[str, Any]) -> str:
        ...
        if current_state:
            ...
            formatted_parts.append(f"- Document has {current_state.get('object_count', 0)} objects")
            
            objects = current_state.get('objects', [])
            if objects:
                formatted_parts.append("- Objects present:")
                for obj in objects[:5]:  # Limit to first 5
                    formatted_parts.append(f"  * {obj.get('name', 'Unknown')} ({obj.get('type', 'Unknown')})")
        ...
        if history:
            ...
            for i, action in enumerate(history[-3:], 1):  # Last 3 actions
                formatted_parts.append(f"{i}. {action.get('command', 'Unknown action')}")
    ...
    def _parse_llm_response(self, response: str, intent_data: Dict[str, Any], 
                           state_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _suggest_next_steps(self, intent_data: Dict[str, Any], state_context: Dict[str, Any]) -> List[str]:
    ...
    def _predict_state_changes(self, command: str, state_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_current_capabilities(self, current_state: Dict[str, Any]) -> Dict[str, bool]:
