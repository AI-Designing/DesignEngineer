...
from .queue_manager import CommandPriority, QueueManager
...
class SystemOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.state_checkpoint_interval = self.config.get(
        ...
        try:
            ...
            if api_client.connect():
                self.command_executor = CommandExecutor(
                    ...
                    state_manager=(
                        self.state_service.state_cache if self.state_service else None
                    ),
            ...
            self.command_generator = StateAwareCommandGenerator(
                llm_client=self.llm_client, state_service=self.state_service
            )
            ...
            self.intent_processor = IntentProcessor(
                state_service=self.state_service, llm_client=self.llm_client
            )
    ...
    def process_user_input(self, user_input: str, session_id: str = None) -> str:
        ...
        try:
            ...
            print(
                f"✅ Intent classified: {intent_data['intent_type']} (confidence: {intent_data['confidence']:.2f})"
            )
            ...
            validation_result = self._validate_command(
                command_data["command"], intent_data
            )
    ...
    def process_user_input_enhanced(
        ...
        try:
            ...
            if self.state_llm_integration:
                print(
                    f"🧠 Using enhanced State-LLM Integration for: {user_input[:50]}..."
                )
                ...
                if self.progress_tracker:
                    ...
                    self.progress_tracker.update_progress(
                        command_id, 1, "Processing intent..."
                    )

                # Use the integrated state-LLM flow
                result = self.state_llm_integration.process_user_request(
                ...
                if self.progress_tracker:
                    if result.get("status") == "success":
                        self.progress_tracker.complete_tracking(
                            command_id, True, "Request completed successfully"
                        )
                    else:
                ...
                if self.websocket_manager and result.get("status") == "success":
                    ...
                    if execution_result.get("status") == "success":
                        self.websocket_manager.send_state_change(
                            {"status": "updated", "session_id": session_id}, session_id
                        )
    ...
    def _process_user_input_fallback(
    ...
    def _validate_command(
        self, command: str, intent_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _create_completion_callback(
        ...
        def callback(command_result):
            ...
            self.performance_metrics["average_processing_time"] = (
                current_avg * (total_commands - 1) + processing_time
            ) / total_commands
            ...
            print(
                f"📊 Command completed in {processing_time:.2f}s (Session: {session_id[:8]})"
            )
            ...
            if state_changes:
                ...
                for change, value in state_changes.items():
                    response_parts.append(
                        f"  • {change.replace('_', ' ').title()}: {value}"
                    )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "queue_status": (
                self.queue_manager.get_queue_status() if self.queue_manager else {}
            ),
            "performance_metrics": self.performance_metrics,
            "components_status": {
                "state_service": (
                    self.state_service.connected if self.state_service else False
                ),
        ...
        def state_monitor():
            while self.system_running:
                try:
                    ...
                    if (
                        current_time - self.last_state_checkpoint
                        ...
                        for session_id in self.active_sessions.keys():
                            try:
                                ...
                                if self.websocket_manager:
                                    latest_state = self.state_service.get_latest_state(
                                ...
                                print(
                                    f"⚠️ Failed to checkpoint session {session_id}: {e}"
                                )

                        self.last_state_checkpoint = current_time
                        print(
                            f"✅ State checkpoint completed for {len(self.active_sessions)} sessions"
                        )
    ...
    def force_state_checkpoint(self, session_id: str = None) -> Dict[str, Any]:
        ...
        sessions_to_checkpoint = (
            [session_id] if session_id else list(self.active_sessions.keys())
        )
        ...
        while True:
            try:
                ...
                if user_input.lower() in ["quit", "exit", "q"]:
                    ...
                    print(
                        f"  Success Rate: {status['performance_metrics']['success_rate']:.2%}"
                    )
                    print(
                        f"  Avg Processing Time: {status['performance_metrics']['average_processing_time']:.2f}s"
                    )
                    ...
                    for sid in self.active_sessions:
                        ...
                        print(
                            f"  • {sid[:8]}: {session_info['command_count']} commands"
                        )

                elif user_input.lower() == "help":
                    print("\\n📖 Available Commands:")
                    print(
                        "  • Natural language: 'create a box', 'analyze current state', etc."
                    )
