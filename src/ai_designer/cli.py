...
class FreeCADCLI:
            ...
            try:
                ...
                if user_input.lower() in ["quit", "exit", "q"]:
                    ...
                    if self.deepseek_enabled:
                        ...
                        print(
                            "❌ DeepSeek R1 is not enabled. Use --deepseek-enabled flag when starting CLI."
                        )
        ...
        try:
            ...
            if "--real" in command:
                ...
                if (
                    ...
                    if result and result.get("status") == "success":
                        ...
                        if freecad_script:
                            ...
                            gui_success = self.persistent_gui.execute_script_in_gui(
                                freecad_script
                            )
                            if gui_success:
                ...
                if result and result.get("status") == "success":
                    ...
                    print(
                        f"❌ REAL EXECUTION FAILED: {result.get('message', 'Execution failed')}"
                    )
                    result["execution_type"] = "REAL_FREECAD"
                    ...
                    if self.progress_tracker:
                        self.progress_tracker.fail_tracking(
                            command_id, result.get("message", "Execution failed")
                        )
            ...
            # Use StateAwareCommandProcessor for all commands
                ...
                result = (
                    self.command_executor.state_aware_processor.process_complex_command(
                        command
                    )
                )
    ...
    def _display_workflow_results(self, result, original_command):
        ...
        if workflow_type == "face_selection":
            ...
            if selected_face:
                print(
                    f"  • Selected Face: {selected_face.get('object_name', 'Unknown')}"
                )
                print(f"  • Face Type: {selected_face.get('face_type', 'Unknown')}")
                print(
                    f"  • Suitability Score: {selected_face.get('suitability_score', 0):.2f}"
                )
    ...
    def _execute_generated_code(self, freecad_code, command_id, source):
        ...
        try:
            ...
            if exec_result.get("status") == "success":
                print(
                    f"✅ DeepSeek R1 SUCCESS: {exec_result.get('message', 'Code executed successfully')}"
                )
                ...
                if self.progress_tracker:
                    self.progress_tracker.complete_tracking(
                        command_id, True, f"DeepSeek R1 {source} completed successfully"
                    )
            else:
                print(
                    f"❌ FreeCAD execution failed: {exec_result.get('message', 'Execution failed')}"
                )
                if self.progress_tracker:
                    self.progress_tracker.fail_tracking(
                        command_id,
                        f"FreeCAD execution failed: {exec_result.get('message')}",
                    )
    ...
    def execute_unified_command(self, command: str, mode: str = "standard"):
        ...
        try:
            ...
            if response.status == "success":
                ...
                # Execute the generated code using the proper command executor
                if self.command_executor and response.generated_code:
                    try:
                        print("\n🔧 Executing in FreeCAD...")

                        # Use the command executor which handles file saving and GUI updates
                        result = self.command_executor.execute(response.generated_code)

                        if result.get("status") == "success":
                            print("✅ Code executed successfully!")

                            # Show file save location if available
                            if result.get("saved_path"):
                                print(f"📁 File saved: {result['saved_path']}")
                            else:
                                print("📁 Check outputs/ directory for saved files")
                            ...
                            if self.persistent_gui and self.enable_persistent_gui:
                                try:
                                    ...
                                    print(f"⚠️ GUI update failed: {gui_e}")
                            ...
                            if self.websocket_manager:
                                try:
                                    self.websocket_manager.send_user_notification(
                                        ...
                                        "success",
                                        getattr(self, "session_id", "cli_session"),
                                    )
                        ...
                        # Still try to save the generated code for reference
                        try:
                            import datetime
                            import os
                            ...
                            with open(code_file, "w") as f:
                                f.write(
                                    f"# Generated by {response.provider.value} at {timestamp}\n"
                                )
...
def main():
    parser = argparse.ArgumentParser(
        description="FreeCAD Command Line Interface with Real-time Features"
    )
    parser.add_argument(
        "--gui", action="store_true", help="Use FreeCAD GUI instead of headless mode"
    )
    ...
    parser.add_argument(
        "--auto-analyze",
        action="store_true",
        help="Automatically analyze state after each command",
    )
    parser.add_argument(
        "--llm-provider",
    ...
    parser.add_argument(
        "--no-websocket",
        action="store_true",
        help="Disable WebSocket server for real-time updates",
    )
    parser.add_argument(
        "--no-persistent-gui",
        action="store_true",
        help="Disable persistent FreeCAD GUI",
    )
    parser.add_argument(
        "--websocket-port",
        type=int,
    ...
    parser.add_argument(
        "--deepseek-enabled",
        action="store_true",
        help="Enable DeepSeek R1 for complex part generation",
    )
    parser.add_argument(
        "--deepseek-mode",
        choices=["reasoning", "fast", "creative", "technical"],
        default="reasoning",
        help="DeepSeek R1 generation mode",
    )
    parser.add_argument(
        "--deepseek-port",
        type=int,
        default=11434,
        help="DeepSeek R1 server port (default: 11434)",
    )
