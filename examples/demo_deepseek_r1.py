...
from ai_designer.deepseek_client import (
    DeepSeekR1Client, 
    DeepSeekConfig, 
    DeepSeekMode,
...
class DeepSeekDemo:
    ...
    def test_connection(self):
        ...
        try:
            ...
            response = self.deepseek_client.generate_complex_part(
                requirements="Create a simple test cube",
        ...
        for command in test_commands:
            ...
            try:
                ...
                results.append({
                    ...
                    'has_freecad_imports': 'import FreeCAD' in code or 'import App' in code
                })
    ...
    def _display_results(self, demo_name: str, response):
        ...
        if response.complexity_analysis:
            ...
            logger.info(f"   Complexity score: {complexity.get('complexity_score', 0):.1f}/10")
            logger.info(f"   Skill level: {complexity.get('skill_level_required', 'unknown')}")
        
        if response.optimization_suggestions:
            logger.info(f"   Optimization suggestions: {len(response.optimization_suggestions)}")
            for i, suggestion in enumerate(response.optimization_suggestions[:3]):
                logger.info(f"     {i+1}. {suggestion}")
        
        # Show a snippet of the generated code
        code_snippet = response.generated_code[:200] + "..." if len(response.generated_code) > 200 else response.generated_code
    ...
    def _display_detailed_reasoning(self, response):
        ...
        for i, step in enumerate(response.reasoning_chain):
            ...
            if step.code_snippet:
                snippet = step.code_snippet[:100] + "..." if len(step.code_snippet) > 100 else step.code_snippet
                logger.info(f"           Code: {snippet}")
        ...
        def _display_final_summary(self, results):
            ...
            try:
                ...
                if hasattr(self, 'integration_manager'):
                    metrics = self.integration_manager.get_integration_metrics()
                    logger.info(f"   Overall DeepSeek usage: {metrics['deepseek_usage_rate']*100:.1f}%")
            except Exception as e:
                logger.warning(f"Could not get final metrics: {e}")
            
            logger.info("=" * 60)
            logger.info("🚀 Demo complete! DeepSeek R1 is ready for complex part generation.")
