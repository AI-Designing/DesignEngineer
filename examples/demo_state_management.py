...
class MockFreeCADStateService:
    ...
    def analyze_and_cache_state(
        self, document_name: str, session_id: str, expiration: int = 3600
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.redis_client.set(
                analysis_key, json.dumps(analysis_data), ex=expiration
            )
    ...
    def get_state_history(
        self, document_name: str, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    def monitor_state_changes(
        self, document_name: str, target_conditions: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def compare_states(self, state_key1: str, state_key2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for key in all_keys:
                ...
                if val1 != val2:
                    differences[key] = {"state1": val1, "state2": val2}

            return {"states_equal": len(differences) == 0, "differences": differences}
    ...
    def export_data(self, document_name: str, format: str = "dict") -> Dict[str, Any]:
        ...
        return {"states": states, "analyses": analyses}
    ...
    def cleanup_old_data(self, older_than_hours: int = 24) -> Dict[str, Any]:
        ...
        return {"success": True, "deleted_states": 0, "deleted_analyses": 0}
...
def demo_monitoring(service):
    ...
    monitoring_result = service.monitor_state_changes(
        "demo_document", target_conditions
    )

    if monitoring_result["success"]:
        print(f"\n📊 Monitoring Results:")
        print(
            f"   All conditions satisfied: {'✅' if monitoring_result['all_conditions_satisfied'] else '❌'}"
        )

        for condition, result in monitoring_result["conditions_met"].items():
            status = "✅" if result["satisfied"] else "❌"
            print(
                f"   {condition}: {status} (Expected: {result['expected']}, Actual: {result['actual']})"
            )
    else:
...
def demo_advanced_features(service):
    ...
    print(
        f"   Exported {len(export_data['states'])} states and {len(export_data['analyses'])} analyses"
    )
