...
from src.ai_designer.core.advanced_prompt_engine import (
    EnhancedLLMIntegration,
    ProblemComplexity,
)
from src.ai_designer.core.enhanced_complex_generator import (
...
class PromptEngineeringDemo:
    ...
    async def _run_single_test(self, test_case: Dict[str, Any]):
        ...
        try:
            ...
            result = self.enhanced_llm.generate_enhanced_freecad_code(
                requirements, {"demo_mode": True, "test_case": test_case["name"]}
            )
    ...
    def _display_generation_results(
        self, result: Dict[str, Any], generation_time: float
    ):
        """Display the results of code generation"""

        print(f"\\n⏱️  Generation Time: {generation_time:.2f} seconds")
        print(
            f"🔧 Generation Method: {result.get('generation_method', 'enhanced_prompt_engineering')}"
        )
        ...
        print(
            f"   ✅ Key Requirements: {len(understanding.get('key_requirements', []))} identified"
        )
        print(
            f"   ⚠️  Potential Challenges: {len(understanding.get('potential_challenges', []))} identified"
        )
        ...
        print(
            f"   🎯 Confidence Level: {implementation.get('confidence_level', 0.0):.1%}"
        )
        print(
            f"   🔍 Complexity Score: {implementation.get('complexity_score', 0.0):.2f}"
        )
        ...
        print(
            f"   ✔️  FreeCAD Compliant: {validation.get('freecad_compliance', False)}"
        )
        print(
            f"   📊 Overall Quality Score: {validation.get('overall_quality_score', 0.0):.1%}"
        )
        ...
        if quality_metrics:
            print(f"\\n📈 QUALITY METRICS:")
            print(
                f"   🎯 Average Quality: {quality_metrics.get('average_quality', 0.0):.1%}"
            )
            print(f"   ✅ Success Rate: {quality_metrics.get('success_rate', 0.0):.1%}")
            print(
                f"   📊 Total Generations: {quality_metrics.get('total_generations', 0)}"
            )
