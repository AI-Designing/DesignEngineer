...
class EnhancedDemo:
    ...
    def demonstrate_mode_comparison(self):
        ...
        requirement = "Create a complex tower with multiple levels and architectural details"
        modes = [
        ...
        for mode, data in results.items():
            print(f"{mode:15} | Time: {data['execution_time']:6.2f}s | Quality: {data['quality_score']:4.2f} | Success: {data['success_rate']:6.1%}")
    ...
    def demonstrate_quality_prediction(self):
        ...
        requirements = [
            ...
            ("Complex architectural design", {"overall_score": 0.85, "aesthetic_quality": 0.8}),
            ("Precision mechanical part", {"geometric_accuracy": 0.98, "manufacturability": 0.9})
        ]
    ...
    def demonstrate_pattern_learning(self):
        ...
        similar_patterns = self.enhanced_generator.pattern_learning.find_similar_patterns(test_requirement)
        
        print(f"📊 Found {len(similar_patterns)} similar patterns")
        for pattern in similar_patterns[:3]:
            print(f"  • Similarity: {pattern.get('similarity_score', 0):.2f}, Success Rate: {pattern.get('success_rate', 0):.2%}")
    ...
    def show_performance_metrics(self):
        ...
        print(f"⏱️ Average Execution Time: {metrics.get('average_execution_time', 0):.2f}s")
    ...
    def _check_quality_targets(self, achieved: QualityMetrics, targets: Dict[str, float]) -> bool:
