    ...
    try:
        ...
        class DemoEnhancedSystem:
            ...
            def process_complex_shape_request(self, user_input: str, session_id: str):
                ...
                complexity_response = self.llm_client.generate_response(
                    "analyze complexity: " + user_input
                )
                complexity_analysis = json.loads(complexity_response)
                print(
                    f"   Complexity Level: {complexity_analysis['overall_complexity']}"
                )
                ...
                decision_response = self.llm_client.generate_response(
                    "make decision: " + user_input
                )
                ...
                feedback_response = self.llm_client.generate_response(
                    "analyze progress: " + user_input
                )
                feedback = json.loads(feedback_response)
                print(f"   Progress: {feedback['progress_percentage']:.1f}%")
                print(
                    f"   Quality: {feedback['quality_assessment']['current_quality']:.2f}"
                )

                # Step 6: Final State
                final_state = self.state_service.get_latest_state(
                    f"session_{session_id}"
                )
                print(f"📊 Final State:")
                print(f"   Objects Created: {final_state['object_count']}")
                print(
                    f"   Quality Score: {final_state['quality_metrics']['geometric_accuracy']:.2f}"
                )
        ...
        state_service = MockStateService()
        command_executor = MockCommandExecutor()
        enhanced_system = DemoEnhancedSystem(
            llm_client, state_service, command_executor
        )
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            result = enhanced_system.process_complex_shape_request(
                user_input=test_case["input"], session_id=test_case["session_id"]
            )
            ...
            if hasattr(result, "get"):
                ...
                if "complexity_analysis" in result:
                    ...
                    print(
                        f"🎯 Complexity achieved: {complexity.get('overall_complexity', 'unknown')}"
                    )
...
def demonstrate_continuous_state_analysis():
    ...
    for state in states:
        print(
            f"   Step {state['step']}: {state['description']} (Objects: {state['objects']})"
        )
