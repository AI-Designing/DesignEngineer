...
def test_phase3_integration():
    ...
    try:
        ...
        print(
            f"   Multi-step workflows available: {processor.multi_step_workflows_available}"
        )
        ...
        for command in complex_commands:
            ...
            print(
                f"   ✅ Complex workflow: {analysis.get('is_complex_workflow', False)}"
            )
            ...
            if any(
                ...
                for keyword in ["bracket", "housing", "assembly", "pattern"]
            ):
                assert (
                    analysis.get("is_complex_workflow", False)
                    or analysis["strategy"] == "complex_workflow"
                )
        ...
        for test_case in test_cases:
            analysis = processor._analyze_workflow_requirements(
                test_case["command"], existing_state
            )
        ...
        if processor.multi_step_workflows_available:
            ...
            workflow_analysis = processor._analyze_workflow_requirements(
                bracket_command, existing_state
            )
            ...
            if workflow_analysis.get("is_complex_workflow", False):
                ...
                print(
                    f"   ✅ Completed steps: {mock_result['completed_steps']}/{mock_result['total_steps']}"
                )
            ...
            print(
                "   ⚠️ Complex workflow orchestrator not available - expected behavior"
            )
        ...
        routing_tests = [
            {"command": "Create a simple box", "expected_strategy": "simple"},
            {
        ...
        for test in routing_tests:
            ...
            analysis = processor._analyze_workflow_requirements(
                test["command"], test_state
            )

            print(f"   Command: '{test['command']}'")
            print(
                f"   ✅ Strategy: {analysis['strategy']} (expected: {test['expected_strategy']})"
            )
...
def test_complexity_scoring():
    ...
    try:
        ...
        for test in complexity_tests:
            ...
            ), f"Score {score} not in range {test['expected_range']}"
