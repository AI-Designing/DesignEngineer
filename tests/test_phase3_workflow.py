...
def test_workflow_orchestrator():
    ...
    try:
        from ai_designer.freecad.workflow_orchestrator import (
            WorkflowOrchestrator, WorkflowStep, WorkflowStepType
        )
        ...
        pattern_steps = orchestrator.decompose_complex_workflow(pattern_command, mock_state)
        
        print(f"   ✅ Pattern decomposed into {len(pattern_steps)} steps")
        for i, step in enumerate(pattern_steps, 1):
            print(f"      Step {i}: {step.description} ({step.step_type.value})")
        
        assert any(step.step_type == WorkflowStepType.PATTERN_CIRCULAR for step in pattern_steps)
        ...
        for step in sorted_steps:
            for dep_id in step.dependencies:
                if dep_id not in executed_step_ids:
                    print(f"   ⚠️ Dependency issue: {step.step_id} depends on {dep_id} but it hasn't been executed yet")
        ...
        execution_result = orchestrator.execute_workflow_steps(sorted_steps, execution_context)
        ...
        for cmd in test_commands:
            if "wide" in cmd and "tall" in cmd:
                ...
                print(f"   ✅ '{cmd}' → Count: {count}, Diameter: {diameter}mm, Spacing: {spacing}mm")
                assert count == 6
                assert diameter == 8.0
                assert spacing == 15.0
        ...
        for command, expected_pattern in pattern_tests.items():
            detected_pattern = orchestrator._identify_workflow_pattern(command.lower(), mock_state)
            print(f"   ✅ '{command}' → Pattern: {detected_pattern}")
            if expected_pattern != "generic_multi_step":  # Allow fallback for some cases
                assert detected_pattern == expected_pattern or detected_pattern == "generic_multi_step"
...
def test_workflow_step_types():
    ...
    try:
        from ai_designer.freecad.workflow_orchestrator import WorkflowStepType, WorkflowStep
        
        # Test all step types are defined
        expected_types = [
            "SKETCH_CREATE", "OPERATION_PAD", "OPERATION_POCKET", "OPERATION_HOLE",
            "FACE_SELECTION", "PATTERN_LINEAR", "PATTERN_CIRCULAR", "PATTERN_MATRIX",
            "FEATURE_FILLET", "FEATURE_CHAMFER", "FEATURE_SHELL", "ASSEMBLY_CONSTRAINT",
            "STATE_VALIDATION"
        ]
