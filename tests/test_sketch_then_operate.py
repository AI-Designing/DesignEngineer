...
class TestSketchThenOperateWorkflow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_api_client.get_document_state.return_value = self.mock_current_state[
            "live_state"
        ]
    ...
    def test_workflow_analysis_cylinder_command(self):
        ...
        analysis = self.processor._analyze_workflow_requirements(
            command, self.mock_current_state
        )
    ...
    def test_preflight_checks_success(self):
        ...
        good_state = {"live_state": {"document_name": "TestDoc", "has_errors": False}}
    ...
    def test_preflight_checks_failure(self):
        ...
        bad_state = {"live_state": {"document_name": None, "has_errors": False}}
    ...
    @patch(
        "ai_designer.freecad.state_aware_processor.StateAwareCommandProcessor._get_current_state"
    )
    ...
    def test_create_circle_sketch_success(self):
        ...
        self.mock_api_client.execute_command.return_value = (
            "SUCCESS: Circle sketch created"
        )
    ...
    def test_create_rectangle_sketch_success(self):
        ...
        # Mock successful execution
        self.mock_api_client.execute_command.return_value = (
            "SUCCESS: Rectangle sketch created"
        )
    ...
    @patch(
        "ai_designer.freecad.state_aware_processor.StateAwareCommandProcessor._ensure_active_body"
    )
    @patch(
        "ai_designer.freecad.state_aware_processor.StateAwareCommandProcessor._create_parametric_sketch"
    )
    @patch(
        "ai_designer.freecad.state_aware_processor.StateAwareCommandProcessor._execute_sketch_operation"
    )
    def test_full_sketch_then_operate_workflow(
        self, mock_execute_op, mock_create_sketch, mock_ensure_body, mock_get_state
    ):
        ...
        mock_get_state.side_effect = [
            self.mock_current_state,  # Initial state
            {
                **self.mock_current_state,
                "live_state": {
                    **self.mock_current_state["live_state"],
                    "active_body": True,
                },
            },  # After body
        ...
        mock_ensure_body.return_value = {
            "status": "success",
            "operation": "ensure_active_body",
        }
        mock_create_sketch.return_value = {
            "status": "success",
        ...
        # Execute workflow
        workflow_analysis = self.processor._analyze_workflow_requirements(
            command, self.mock_current_state
        )
        result = self.processor._process_sketch_then_operate_workflow(
            command, self.mock_current_state, workflow_analysis
        )
...
class TestComplexityAnalysis(unittest.TestCase):
    ...
    def test_step_estimation_sketch_then_operate(self):
        ...
        steps = self.processor._estimate_step_count(
            "Create a cylinder", "sketch_then_operate"
        )
    ...
    def test_complexity_scoring(self):
        ...
        complex_command = (
            "Design a complex gear housing with mounting brackets and ventilation holes"
        )

        simple_score = self.processor._calculate_complexity_score(
            simple_command, {"object_count": 0}
        )
        complex_score = self.processor._calculate_complexity_score(
            complex_command, {"object_count": 2}
        )
