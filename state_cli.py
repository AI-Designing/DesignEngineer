...
Examples:
  ...
  %(prog)s export --document "MyProject" --output analysis.json
    ...
    # Analyze command
    analyze_parser = subparsers.add_parser('analyze', help='Analyze FreeCAD state and cache results')
    ...
    analyze_parser.add_argument('--expiration', type=int, default=3600,
    ...
    list_analyses_parser = subparsers.add_parser('list-analyses', help='List cached analyses')
    ...
    monitor_parser.add_argument('--conditions', required=True,
    ...
    clear_parser.add_argument('--document', help='Document name (if not specified, clears all)')
...
def handle_monitor(service, args):
    ...
    if result['success']:
        print(f"\n📊 Monitoring Results:")
        print(f"All conditions satisfied: {'✅' if result['all_conditions_satisfied'] else '❌'}")
        
        for condition, check in result['conditions_met'].items():
            status = '✅' if check['satisfied'] else '❌'
            print(f"  {condition}: {status} (Expected: {check['expected']}, Actual: {check['actual']})")
    else:
